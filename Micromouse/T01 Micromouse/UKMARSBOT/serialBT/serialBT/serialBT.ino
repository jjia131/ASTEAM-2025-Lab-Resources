...
// October 2022
// found Back light on 74, off 75
//       Head light on 72, off 73
//
...
void setup() {
  ...
  Serial.println(F("SERIAL BT TEST\n"));
  motorSetup();
  updateTime = millis() + updateInterval;

   pinMode(LED_BACK,OUTPUT);
   digitalWrite(LED_BACK,0); // turn on back light
}
...
void loop() {
  ...
  if (getFunctionSwitch() == 16) {
    ...
    */
    ...
    {
       command = Serial.read();  // see BT remote control
       Serial.println(command);

       switch(command)
       {
        case 'F':  setMotorPWM(200,200); break;
        case 'B':  setMotorPWM(-200, -200); break;
        case 'S':  setMotorPWM(0, 0); break;
        case 'L':   setMotorPWM(-127, 127); break;
        case 'R':   setMotorPWM(127, -127); break;
        case 74 : digitalWrite(LED_BACK,0); break;
        case 75 :  digitalWrite(LED_BACK,1); break;
